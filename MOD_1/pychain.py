...
class Record:
    """
    class record to have transaction information on each entry
    """
    sender:str
    receiver:str
    amount:float
...
class Block:
    ...
    record: Record  # each record property will be of type(record) = Record
    ...
    def __str__(self):
        """
        adding a str method to fix the issue when printing ledger objects as dataframe pukes the Record object as an error
        this is autocalled when added to the list of dropdown choices
        """
        return f"Record: {self.record.sender},{self.record.receiver},{str(self.record.amount)}"
        ...
        #      'n_once':{str(self.nonce)},
        # }

    def __repr__(self):
        """
        adding a repr method to fix the issue when printing ledger objects as dataframe pukes the Record object as an error

        easier when printing to the console to help me debug.
        you can add other properties, here just worried about the record of each block
        """
        return f"Record repr: {self.record.sender},{self.record.receiver},{str(self.record.amount)}"
...
def setup():
    ...
    return PyChain([Block(record=Record("Genesis","Genesis","100"),creator_id=0)])
...
# input_data = st.text_input("Block Data") # ELIMINATE PER INSTRUCTIONS, SAVING CODE AS PRECAUTION

# @TODO:
# Add an input area where you can get a value for `sender` from the user.
# YOUR CODE HERE
st.title="Pychain App"
sender,receiver,amount = st.columns(3)

sender=sender.text_input("Sender:",key="sender")
receiver = receiver.text_input("Receiver:",key="receiver")
amount = amount.text_input("Amount to send:",key="amount",value="1000")

# st.number_input("Enter A Number",format="{:,}",value=10_000.00)
...
print("The Chain: ",pychain.chain)

def build_dataframe(obj):
    """
    function takes in a the chain to build dataframa and deal with hash value which
    has an issue printing to the webpage via the write method, maybe try the dataframe method later, but this way
    it is a lot cleaner
    """
    data_f = {
        'sender':[],
    ...
    # print(data_f)
    for r in obj:
        # append records to dataframe
        data_f['sender'].append(r.record.sender)
        data_f['receiver'].append(r.record.receiver)
        data_f['amount'].append(r.record.amount)
        data_f['creator'].append(r.creator_id)
        data_f['timestamp'].append(r.timestamp)
        data_f['prev_hash'].append(str(r.prev_hash))
        data_f['nonce'].append(r.nonce)
    return pd.DataFrame(data_f)

        
pychain_df = build_dataframe(pychain.chain)
# print(pychain_df)


# print(pychain_df.columns)

# print(pychain_df['record'][0])
# print(pychain_df['nonce'])
st.dataframe(pychain_df.style.format(formatter={('nonce'): "{:,}",
                           ('amount'): lambda v: "$ {:,.2f}".format(float(v))
                          }))
...
selected_block = st.sidebar.selectbox(
    "Which block would you like to see?", pychain.chain  # using the str( i get the repr version of the class)
)
