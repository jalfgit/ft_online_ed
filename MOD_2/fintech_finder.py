...
# ja code
# i like to import the whole module as all functions are there and lets
# me identify where the function is located


# NOTE FROM JA
# DOTENV not working and had to create environment variables via the console
# MNEMONIC and WEB3_INFURA created before the streamlit command
# export VAR='VALUE'


import crypto_wallet as cw # cw is short, and still lets me know functions are in other module
...
# YOUR CODE HERE
# ja code here
account = cw.generate_account() # no params as cw imports from ENV
...
bal = cw.get_balance(account.address) # account is the address
st.sidebar.markdown(f'<b>{bal:.2f}</b>',unsafe_allow_html=True)
...
# YOUR CODE HERE
# ja code here
st.sidebar.markdown(f"Wage expense is: {wage:.2f}")
...
if st.sidebar.button("Send Transaction"):
    ...
    # YOUR CODE HERE
    # ja code
    transaction_hash = cw.send_transaction(account,candidate_database[person][1],wage)
